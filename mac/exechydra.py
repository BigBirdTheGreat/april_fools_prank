...
nameoffile = "execwatchdog" #name of resurrecting file
...
dog_path = os.path.join(base_dir, nameoffile) # CHANGE WHEN BUNDLING
...
def watchthedog():    
    ...
    def find_or_start_process(process_name):
        ...
        if process is None:
            process = subprocess.Popen([dog_path], stdin=subprocess.DEVNULL, preexec_fn=os.setsid)
            
        firsttime = False  #sets firstime to false
        
        return process

    process_name = "execwatchdog"
